...
class snake_env(): 
    ...
    def __init__ (self,snake_size=20,win_width=20*30,win_height=20*20,\
                  ...
                  starving_limit=100,starving_only_1=False, zerosum=True, starting_tail=0,starting_tail2=0,\
                  give_food_pos=False,food_pos=[0,0],allow_180=False):
        "Given Variables"
        ...
        self.starting_tail=starting_tail
        self.starting_tail2=starting_tail2
        self.starving_only_1=starving_only_1
        self.give_food_pos=give_food_pos
        self.food_pos=food_pos
        self.allow_180=allow_180
    def setup(self):
        self.food_num=0
        self.starting_tail_num=self.starting_tail
        self.starting_tail2_num=self.starting_tail2
        self.starving1=0
        self.starving2=0
        "-------Create Snakes and food------"         
        self.snake=self.snakeparts(self.snake_size*1,self.snake_size*2,self.snake_size,self.snake_size,self.snake_size,0) #pos_x,pos_y,witdh,height,v_x,v_y
        ...
        self.food=self.foods(self.snake_size,self.snake_size,self.win_width,\
                                 ...
                                 self.snake.x,self.snake.y,self.parts,self.give_food_pos,self.food_pos,self.food_num)  
        self.food_num+=1
        if self.multiplayer:
            self.snake2=self.snakeparts(self.snake_size*0,self.snake_size*6,self.snake_size,self.snake_size,self.snake_size,0)
        self.parts2=list()
    ...
    def step(self,action, action2=0):
        ...
        self.reward_col=0
        "---Computation---"
        if self.starting_tail_num >0:
            self.starting_tail_num-=1
            self.snake.add_part=True
        if self.add_food:
            self.add_food=False
            self.food=self.foods(self.snake_size,self.snake_size,self.win_width,\
                                 self.win_height,self.snake_size,self.snake.num_parts,\
                                 self.snake.x,self.snake.y,self.parts,self.give_food_pos,self.food_pos,self.food_num)     #creates new food if food has been eaten       
            self.food_num+=1
        ...
        if self.starving_mode:
            if self.starving1==self.starving_limit:
                ...
                self.done=True        
        if self.multiplayer:
            if self.starving2==self.starving_limit:
                self.snake2.lost=True
                self.reward2=-4
                self.done=True
            if self.starting_tail2_num >0:
                self.starting_tail2_num-=1
                self.snake2.add_part=True
            if not self.starving_only_1:
                self.starving2+=1
            if self.special_food_mode:
        ...
        if self.multiplayer == False:
            if self.simple_observation:
                ...
                ld=5
                vision=4
                for k in range(len(self.parts)-1): #last part invisible because it's going to move
                    for i in range(vision): #parts vision 4
                        if self.parts[k].y==self.snake.y and self.parts[k].x==self.snake.x+(i+1)*self.snake_size:                                
                            if i < r:
                                r=i
                                self.pov_ob[0]=(vision-r)/vision #right
                            break
                        if self.parts[k].x==self.snake.x and self.parts[k].y==self.snake.y+(i+1)*self.snake_size:
                            if i < d:
                                d=i
                            self.pov_ob[1]=(vision-d)/vision #down
                            break
                        if self.parts[k].x==self.snake.x and self.parts[k].y==self.snake.y-(i+1)*self.snake_size:
                            if i < u:
                                u=i
                            self.pov_ob[2]=(vision-u)/vision #up
                            break
                        if self.parts[k].y==self.snake.y and self.parts[k].x==self.snake.x-(i+1)*self.snake_size:
                            if i < l:
                                l=i
                            self.pov_ob[3]=(vision-l)/vision #left
                            break
                        if self.parts[k].y==self.snake.y-(i+1)*self.snake_size and self.parts[k].x==self.snake.x+(i+1)*self.snake_size:                                
                            if i < ru:
                                ru=i
                                self.pov_ob[12]=(vision-ru)/vision #rightup
                            break
                        if self.parts[k].y==self.snake.y+(i+1)*self.snake_size and self.parts[k].x==self.snake.x+(i+1)*self.snake_size:                                
                            if i < rd:
                                rd=i
                                self.pov_ob[13]=(vision-rd)/vision #rightdown
                            break
                        if self.parts[k].y==self.snake.y-(i+1)*self.snake_size and self.parts[k].x==self.snake.x-(i+1)*self.snake_size:
                            if i < lu:
                                lu=i
                            self.pov_ob[14]=(vision-lu)/vision #leftup
                            break                       
                        if self.parts[k].y==self.snake.y+(i+1)*self.snake_size and self.parts[k].x==self.snake.x-(i+1)*self.snake_size:
                            if i < ld:
                                ld=i
                            self.pov_ob[15]=(vision-ld)/vision #leftdown
                            break                  
            ...
            if self.POV_mode:
                    ...
                    for i in range(vision): #parts vision 4    
                        ...
                        if i>r:
                            if self.parts[k].y==self.snake.y and self.parts[k].x==self.snake.x+(vision-i)*self.snake_size:
                                self.pov_ob[0]=(i+1)/vision
                                r=i
                        if i>d:
                            if self.parts[k].x==self.snake.x and self.parts[k].y==self.snake.y+(vision-i)*self.snake_size:
                                self.pov_ob[1]=(i+1)/vision
                                d=i
                        if i>u:
                            if self.parts[k].x==self.snake.x and self.parts[k].y==self.snake.y-(vision-i)*self.snake_size:
                                self.pov_ob[2]=(i+1)/vision
                                u=i
                        if i>l:
                            if self.parts[k].y==self.snake.y and self.parts[k].x==self.snake.x-(vision-i)*self.snake_size:
                                self.pov_ob[3]=(i+1)/vision
                                l=i
                        if i>ru:
                            if self.parts[k].y==self.snake.y-(vision-i)*self.snake_size and self.parts[k].x==self.snake.x+(vision-i)*self.snake_size:
                                self.pov_ob[12]=(i+1)/vision
                                ru=i
                        if i>rd:
                            if self.parts[k].y==self.snake.y+(vision-i)*self.snake_size and self.parts[k].x==self.snake.x+(vision-i)*self.snake_size: 
                                self.pov_ob[13]=(i+1)/vision
                                rd=i
                        if i>lu:
                            if self.parts[k].y==self.snake.y-(vision-i)*self.snake_size and self.parts[k].x==self.snake.x-(vision-i)*self.snake_size:
                                self.pov_ob[14]=(i+1)/vision
                                lu=i
                        if i>ld:
                            if self.parts[k].y==self.snake.y+(vision-i)*self.snake_size and self.parts[k].x==self.snake.x-(vision-i)*self.snake_size:
                                self.pov_ob[15]=(i+1)/vision
                                ld=i
                            "snake2"
                        if i>r2:
                            if self.parts[k].y==self.snake2.y and self.parts[k].x==self.snake2.x+(vision-i)*self.snake_size or self.snake.x==self.snake2.x+(vision-i)*self.snake_size:
                                self.pov_ob[20]=(i+1)/vision
                                r2=i
                        if i>d2:
                            if self.parts[k].x==self.snake2.x and self.parts[k].y==self.snake2.y+(vision-i)*self.snake_size or self.snake.y==self.snake2.y+(vision-i)*self.snake_size:
                                self.pov_ob[21]=(i+1)/vision
                                d2=i
                        if i>u2:
                            if self.parts[k].x==self.snake2.x and self.parts[k].y==self.snake2.y-(vision-i)*self.snake_size or self.snake.y==self.snake2.y-(vision-i)*self.snake_size:
                                self.pov_ob[22]=(i+1)/vision
                                u2=i
                        if i>l2:
                            if self.parts[k].y==self.snake2.y and self.parts[k].x==self.snake2.x-(vision-i)*self.snake_size or self.snake.x==self.snake2.x-(vision-i)*self.snake_size:
                                self.pov_ob[23]=(i+1)/vision
                                l2=i
                        if i>ru2:
                            if self.parts[k].y==self.snake2.y-(vision-i)*self.snake_size and self.parts[k].x==self.snake2.x+(vision-i)*self.snake_size:
                                self.pov_ob[32]=(i+1)/vision
                                ru2=i
                        if i>rd2:
                            if self.parts[k].y==self.snake2.y+(vision-i)*self.snake_size and self.parts[k].x==self.snake2.x+(vision-i)*self.snake_size: 
                                self.pov_ob[33]=(i+1)/vision
                                rd2=i
                        if i>lu2:
                            if self.parts[k].y==self.snake2.y-(vision-i)*self.snake_size and self.parts[k].x==self.snake2.x-(vision-i)*self.snake_size:
                                self.pov_ob[34]=(i+1)/vision
                                lu2=i
                        if i>ld2:
                            if self.parts[k].y==self.snake2.y+(vision-i)*self.snake_size and self.parts[k].x==self.snake2.x-(vision-i)*self.snake_size:
                                self.pov_ob[35]=(i+1)/vision
                                ld2=i
                for k in range(len(self.parts2)):
                    for i in range(vision):
                        "snake1"
                        if i>r:
                            if self.parts2[k].y==self.snake.y and self.parts2[k].x==self.snake.x+(vision-i)*self.snake_size or self.snake2.x==self.snake.x+(vision-i)*self.snake_size:
                                self.pov_ob[0]=(i+1)/vision
                                r=i
                        if i>d:
                            if self.parts2[k].x==self.snake.x and self.parts2[k].y==self.snake.y+(vision-i)*self.snake_size or self.snake2.y==self.snake.y+(vision-i)*self.snake_size:
                                self.pov_ob[1]=(i+1)/vision
                                d=i
                        if i>u:
                            if self.parts2[k].x==self.snake.x and self.parts2[k].y==self.snake.y-(vision-i)*self.snake_size or self.snake2.y==self.snake.y-(vision-i)*self.snake_size:
                ...
                "Seeing First Part:"
                "snake1"
                if self.snake2.y==self.snake.y and self.snake2.x==self.snake.x+1*self.snake_size:
                    self.pov_ob[0]=1
                if self.snake2.x==self.snake.x and self.snake2.y==self.snake.y+1*self.snake_size:
                    self.pov_ob[1]=1
                if self.snake2.x==self.snake.x and self.snake2.y==self.snake.y-1*self.snake_size:
                    self.pov_ob[2]=1
                if self.snake2.y==self.snake.y and self.snake2.x==self.snake.x-1*self.snake_size:
                    self.pov_ob[3]=1
                if self.snake2.y==self.snake.y-1*self.snake_size and self.snake2.x==self.snake.x+1*self.snake_size:
                    self.pov_ob[12]=1
                if self.snake2.y==self.snake.y+1*self.snake_size and self.snake2.x==self.snake.x+1*self.snake_size: 
                    self.pov_ob[13]=1
                if self.snake2.y==self.snake.y-1*self.snake_size and self.snake2.x==self.snake.x-1*self.snake_size:
                ...
                if self.snake2.y==self.snake.y+1*self.snake_size and self.snake2.x==self.snake.x-1*self.snake_size:
                    self.pov_ob[15]=1
                "snake2"
                if self.snake.y==self.snake2.y and self.snake.x==self.snake2.x+1*self.snake_size:
                    self.pov_ob[20]=1
                if self.snake.x==self.snake2.x and self.snake.y==self.snake2.y+1*self.snake_size:
                    self.pov_ob[21]=1
                if self.snake.x==self.snake2.x and self.snake.y==self.snake2.y-1*self.snake_size:
                    self.pov_ob[22]=1
                if self.snake.y==self.snake2.y and self.snake.x==self.snake2.x-1*self.snake_size:
                ...
                if self.special_food_active:
                    ...
                    if self.snake.y<self.specialfood.y:
                        ...
                        self.pov_ob[19]=1-(self.snake.y-self.specialfood.y)/(self.win_height) 
                    "Snake2"
                    if self.snake2.x<self.specialfood.x:
                    ...
                    if self.snake2.y<self.specialfood.y:
    ...
    class foods:
        def __init__(self,width,height,win_width,win_height,snake_size,num_parts,x,y,parts,give_food_pos,food_pos,food_num):
            self.width=width
            self.height=height
            new_food=True
            if give_food_pos==False:
                while new_food:
                    x=np.random.randint(0,(win_width-snake_size)//snake_size)*snake_size
                    y=np.random.randint(0,(win_height-snake_size)//snake_size)*snake_size
                    new_food=False
                ...
                x=food_pos[0,food_num]
                y=food_pos[1,food_num]
            self.x=x
            self.y=y
        def draw(self,win):    
