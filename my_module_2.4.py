...
class snake_env(): 
    ...
    def __init__ (self,snake_size=20,win_width=20*30,win_height=20*20,\
                  ...
                  starving_limit=100,zerosum=True):
        "Given Variables"
    ...
    def setup(self):
        ...
        if self.multiplayer:
            self.snake2=self.snakeparts(self.snake_size*0,self.snake_size*2,self.snake_size,self.snake_size,self.snake_size,0)
        self.parts2=list()
                ...
                for k in range(len(self.parts)-1):
                    for i in range(vision): #parts vision 4    
                        ...
                        if self.parts[k].y==self.snake2.y+(vision-i)*self.snake_size and self.parts[k].x==self.snake2.x-(vision-i)*self.snake_size:
                            ...
                            ld2=i
                for k in range(len(self.parts2)-1):
                    for i in range(vision):
                        "snake1"
                        if i>r:
